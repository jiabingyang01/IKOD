...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path):
        ...
        self.captions = {imid: None for imid in imids}

        self.coco_path = coco_path

        #read in synonyms
        synonyms = open('./synonyms.txt').readlines()
    ...
    def get_annotations_from_captions(self):
        ...
        for i, annotation in enumerate(caption_annotations):
            ...
            if imid in self.imid_to_objects:
                ...
                self.captions[imid] = annotation['caption']
        print ("\n")
    ...
    def compute_chair(self, cap_file):
        ...
        gts = {imid: None for imid in imids}
        res =  {imid: None for imid in imids}
        imgIds = []
        for i, cap_eval in enumerate(caps): 
            res[cap_eval['image_id']] = cap_eval['caption']
            gts[cap_eval['image_id']] = self.captions[cap_eval['image_id']]
            imgIds.append(cap_eval['image_id'])
        # print(res)
        # print(gts)
        # coco_eval = COCOEvalCap(imgIds, gts, res)
        # scores = coco_eval.evaluate()
        ...
        # for key in res:
        #     # Each ID's res and gts are fed into the BERT score function
        #     P, R, F1 = score([res[key]], [gts[key]], lang="en", verbose=True)
        #
        #     # Convert tensors to Python floats and store them
        #     precision_list.append(P.item())
        #     recall_list.append(R.item())
        #     f1_list.append(F1.item())
        #     ids.append(key)
        #
        # # Creating a DataFrame to nicely present the scores for each ID
        # bert_scores ={
        #     "bert_Precision": precision_list.mean(),
        #     "bert_Recall": recall_list.mean(),
        #     "bert_F1 Score": f1_list.mean()
        # }
        #
        # print(bert_scores)
        for i, cap_eval in enumerate(caps):
            ...
            for word, node_word, idx in zip(words, node_words, idxs):
                if node_word not in gt_objects and word not in setlist:
                    setlist.append(word)
        ...
        output['overall_metrics'] = {
                                     'CHAIRs': chair_s,
                                     'CHAIRi': chair_i}
        # output['overall_metrics'].update(scores)
        # output['overall_metrics'].update(bert_scores)
...
if __name__ == '__main__':
    ...
    #llava_cocotrain_rewrite_context_idnt1
    # rewrite_bert100_des_sample_context_idn_wt_ab.jsonl
    parser.add_argument("--cap_file", type=str, default='./file_json/VCD/experiments/output/llava15_chair_answers_our_5_greedy_seed100.json')
    parser.add_argument("--annotation_path", type=str, default='/data/public_models/eval/models/AGLA/experiments/data/CHAIR/annotations')
    args = parser.parse_args()
    #--cap_file
    #F:\zamodel\bert100_rewrite_sample_context_idnt1.json
    _, imids, _ = load_generated_captions(args.cap_file)

    evaluator = CHAIR(imids, args.annotation_path)
    evaluator.get_annotations()
    cap_dict = evaluator.compute_chair(args.cap_file)

    # print_metrics(cap_dict)
    print(cap_dict['overall_metrics'])
    save_hallucinated_words('llava15_chair_answers_our_5_greedy_seed100_result.jsonl', cap_dict)
